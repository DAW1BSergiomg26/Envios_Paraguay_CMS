...
---
## 📌 Estado Git Actual
- **Rama:** `main` (estable) — **sincronizada con `origin/main`** en `faacbbd` (2026-08-06). Sin push ni merge sin confirmación explícita del usuario.
- **HEAD:** `faacbbd` — `fix(admin): resolve LazyInitializationException on /admin/documentos with EntityGraph and fix modal hidden CSS rule`.
- **Sprint actual (2026-08-06):** cirugía CSS final — cajas blancas eliminadas (`/casa`, `/contacto` → `#1B4D3B`), calendario `/reservas` oscuro (`#123524`), nav a la izquierda (`flex-start`, 1.15rem/600), logo tricolor ya unificado. Fix 500 real en `/admin/documentos` (`LazyInitializationException` por proxy LAZY de `cliente` → `@EntityGraph` en `EnvioTrackingRepository`). Fix modal `/admin/imports` (`.modal[hidden]{display:none !important}`). Cierre DevOps: HEALTHCHECK Dockerfile `wget`→`curl`.
- **Suite completa:** **267 tests, 0 fallos, BUILD SUCCESS** (local, pom Java 25 restaurado) y **CI/CD en verde en la nube** (runs `31090036075`, `31091604721` y `31095169953`: test suite MySQL 8 + Redis 7 + Docker build + smoke `/actuator/health` → UP, `conclusion=success` en ambos jobs).
