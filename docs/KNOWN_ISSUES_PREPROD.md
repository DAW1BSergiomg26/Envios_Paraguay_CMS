...
|---|----------|---------|-----------|-------------------|------------------|
| 1 | **CasaRuralApplication naming legacy** | El nombre de la clase principal y el artifactId (`casarural`) no reflejan la marca Monteastur. Esto causa confusión en logs, métricas y despliegue. | Media | Renombrar clase a `MonteasturApplication`, actualizar `artifactId` en `pom.xml` y refrescar todo el proyecto. | Fase 19.2 |
| 2 | **Package `com.grupb2.casarural` legacy** | Todo el código fuente está bajo un package que no corresponde al dominio ni a la marca actual. Dificulta el mantenimiento a largo plazo. | Baja | Refactor global: cambiar a `com.monteastur.envios` o similar. Herramienta: IDE refactor + rename package. **Riesgo alto de romper imports.** | Fase 19.2 |
